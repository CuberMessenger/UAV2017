...
void FTC_RC::Cal_Command(void) {
	int32_t tmp, tmp2;
	for (u8 i = 0; i < 3; i++) { //处理ROLL,PITCH,YAW这三个轴的数据
...
void FTC_RC::CheckAUX(void) {
	...
	static uint8_t jump_delay = 0;
	static bool ready_to_jump = false;

	if (fc.jumping) {
		jump_delay = 0;
		ready_to_jump = false;
	}
	else if (jump_delay < 255)
		jump_delay++;
	else
		jump_delay = 0;

	if (rc.rawData[AUX1] > 1499 && rc.rawData[AUX1] < 2100) { //1499 to be discussed
	...
	if (jump_delay == 250) { // 5s after pre-jumping (5s for adjusting)
		ready_to_jump = true;
		jump_delay = 0;
	}

	if (rcDelayCommand == 50) { //hold for 1s
		if (ready_to_jump)
			fc.Jump();
